...
class ThumbnailImageFieldFile(ImageFieldFile):
    #기존 이미지 파일명을 기준으로 썸네일 이미지 파일명 만듬
    def _add__thumb(s):
        parts=s.split(".")
    ...
    def save(self, name, content, save=True):
        super().save(name, content, save) #부모클래스의 save()메소드 호출해 원본이미지 저장

        img= Image.open(self.path)
        size = (self.field.thumb_width, self.field.thumb_height)
        img.thumbnail(size) #썸네일 이미지 만드는 함수
        background = Image.now('RGB', size, (255,255,255))#배경 흰색으로 만듬
